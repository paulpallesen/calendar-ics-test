...
logger.info(f"First 5 rows: {df.head()}")  # Debug: Log the first 5 rows

Path('public').mkdir(parents=True, exist_ok=True)  # Ensure public directory exists

cal = Calendar()
cal.extra.append("X-WR-CALNAME:Test Calendar")  # Use string format
cal.extra.append("X-WR-TIMEZONE:Australia/Sydney")  # Use string format
